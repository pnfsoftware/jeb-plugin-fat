 ...
 * "super floppy" standard. This means that the device will be formatted so that
 * it does not contain a partition table. Instead, the entire device holds a
 * single FAT file system.
 * </p>
 * <p>
...
public final class SuperFloppyFormatter {
     ...
     * @param device
     * @throws IOException
     *             on error accessing the specified {@code device}
     */
     ...
     * @throws IOException
     *             on error creating the formatter
     */
     ...
     * 
     * @param label
     *            the new file system label, may be {@code null}
     * @return this {@code SuperFloppyFormatter}
    ...
    public FatFileSystem format() throws IOException {
        ...
        if(fatType == FatType.FAT32) {
            ...
            final int rootDirEntries = rootDirectorySize(device.getSectorSize(), totalSectors);
        ...
        final FatLfnDirectory rootDir = new FatLfnDirectory(rootDirStore, fat, false);
    ...
    private int sectorsPerFat(int rootDirEntries, int totalSectors) throws IOException {
        ...
        final int rootDirSectors = ((rootDirEntries * 32) + (bps - 1)) / bps;
        final long tmp1 = totalSectors - (this.reservedSectors + rootDirSectors);
     ...
     * @throws IOException
     *             on error determining the device's size
     */
     ...
     *
     * @param fatType
     *            the desired {@code FatType}
     * @return this {@code SuperFloppyFormatter}
     * @throws IOException
     *             on error setting the {@code fatType}
    ...
    private int sectorsPerCluster32() throws IOException {
        ...
        return sectors > 67108864 ? 64 : sectors > 33554432 ? 32 : sectors > 16777216 ? 16 : sectors > 532480 ? 8 : 1;
    }
    ...
    private int sectorsPerCluster16() throws IOException {
        ...
        return sectors > 2097152 ? 64 : sectors > 1048576 ? 32 : sectors > 524288 ? 16 : sectors > 262144 ? 8
                : sectors > 32680 ? 4 : 2;
    }
    ...
    private int sectorsPerCluster12() throws IOException {
        ...
        while(sectors / result > Fat16BootSector.MAX_FAT12_CLUSTERS) {
            result *= 2;
            if(result * device.getSectorSize() > 4096)
