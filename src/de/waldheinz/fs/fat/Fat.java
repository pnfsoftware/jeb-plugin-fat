...
final class Fat {
     ...
     *
     * @param bs
     *            the boot sector specifying the {@code Fat} layout
     * @param fatNr
     *            the number of the {@code Fat} to read
    ...
    public static Fat read(BootSector bs, int fatNr) throws IOException, IllegalArgumentException {
        ...
        if(fatNr > bs.getNrFats()) {
            throw new IllegalArgumentException("boot sector says there are only " + bs.getNrFats()
     ...
     *            the boot sector specifying the {@code Fat} layout
     * @param fatNr
     *            the number of the {@code Fat} to create
    ...
    private Fat(BootSector bs, long offset) throws IOException {
        ...
        if(lastClusterIndex > entries.length)
            throw new IOException("file system has " + lastClusterIndex + "clusters but only " + entries.length
                    + " FAT entries");
    }
    ...
    private void init(int mediumDescriptor) {
        entries[0] = (mediumDescriptor & 0xFF) | (0xFFFFF00L & fatType.getBitMask());
     ...
     * 
     * @param offset
     *            the byte offset where to read the FAT from the device
     * @throws IOException
     *             on read error
     */
     ...
     * 
     * @param offset
     *            the device offset where to write the FAT copy
     * @throws IOException
     *             on write error
     */
    ...
    public long allocNew() throws IOException {
        ...
        if(entryIndex < 0) {
            throw new IOException("FAT Full (" + (lastClusterIndex - FIRST_CLUSTER) + ", " + i + ")"); // NOI18N
        }
     ...
     * 
     * @param nrClusters
     *            when number of clusters to allocate
     * @return long
     * @throws IOException
     *             if there are no free clusters
     */
     ...
     * 
     * @param cluster
     *            a cluster from a chain where the new cluster should be
     *            appended
