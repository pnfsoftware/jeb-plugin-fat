...
final class ClusterChain extends AbstractFsObject {
     ...
     *
     * @param fat
     *            the {@code Fat} that holds the new chain
     * @param readOnly
     *            if the chain should be created read-only
     */
     ...
     *
     * @return the chain's first cluster, which may be 0 if this chain does not
     ...
     * {@code ClusterChain} can only contain full clusters, the new size will
     * always be a multiple of the cluster size.
     *
     * @param size
     *            the desired number of bytes the can be stored in this
     *            {@code ClusterChain}
     * @return the true number of bytes this {@code ClusterChain} can contain
     * @throws IOException
     *             on error setting the new size
     ...
     *
     * @param nrClusters
     *            the new number of clusters this chain should contain, must be
     *            {@code >= 0}
     * @throws IOException
     *             on error updating the chain length
     * @see #setSize(long)
     */
    public void setChainLength(int nrClusters) throws IOException {
        if(nrClusters < 0)
            throw new IllegalArgumentException("negative cluster count"); // NOI18N
    ...
    /**
     * Writes data to this cluster chain, possibly growing the chain so it can
     * store the additional data. When this method returns without throwing an
     * exception, the buffer's {@link ByteBuffer#position() position} will equal
     * it's {@link ByteBuffer#limit() limit}, and the limit will not have
     * changed. This is not guaranteed if writing fails.
     *
     * @param offset
     *            the offset where to write the first byte from the buffer
     * @param srcBuf
     *            the buffer to write to this {@code ClusterChain}
     * @throws IOException
     *             on write error
     */
