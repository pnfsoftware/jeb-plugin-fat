...
public final class FatLfnDirectoryEntry extends AbstractFsObject implements FsDirectoryEntry {
    ...
    FatDirectoryEntry[] compactForm() {
        ...
        if(ShortName.canConvert(fileName) && ShortName.get(fileName).asSimpleString().equals(fileName)) {
            /*
             * do not add unnecessary LFN when the short name matches the
             * original name
             */
        ...
        final FatDirectoryEntry[] entries = new FatDirectoryEntry[totalEntrySize];
        ...
        for(int i = totalEntrySize - 2; i > 0; i--) {
            entries[i] = createPart(type, fileName.substring(j * 13, j * 13 + 13), j + 1, checkSum, false);
            j++;
        }

        entries[0] = createPart(type, fileName.substring(j * 13), j + 1, checkSum, true);
