...
abstract class AbstractDirectory {
     ...
     *
     * @param capacity
     *            the initial capacity of the new instance
     * @param readOnly
     *            if the instance should be read-only
     * @param isRoot
     *            if the new {@code AbstractDirectory} represents a root
     *            directory
     */
    ...
    /**
     * Gets called when the {@code AbstractDirectory} must read it's content off
     * the backing storage. This method must always fill the buffer's remaining
     * space with the bytes making up this directory, beginning with the first
     * byte.
     *
     * @param data
     *            the {@code ByteBuffer} to fill
     * @throws IOException
     *             on read error
     */
    ...
    /**
     * Returns the number of the cluster where this directory is stored. This is
     * important when creating the ".." entry in a sub-directory, as this entry
     * must poing to the storage cluster of it's parent.
     *
     ...
     *
     * @param entryCount
     *            the new number of entries this directory needs to store
     * @throws IOException
     *             on write error
     * @throws DirectoryFullException
     *             if the FAT12/16 root directory is full
     * @see #sizeChanged(long)
     * @see #checkEntryCount(int)
     */
    protected abstract void changeSize(int entryCount) throws DirectoryFullException, IOException;
     ...
     *
     * @param newSize
     *            the new storage space for the directory in bytes
     * @see #changeSize(int)
     */
     ...
     * number of "real" entries in this directory, possibly plus one if a volume
     * label is set.
     * 
    ...
    public void flush() throws IOException {
        ...
        final ByteBuffer data = ByteBuffer.allocate(getCapacity() * FatDirectoryEntry.SIZE);
        ...
        if(this.volumeLabel != null) {
            final FatDirectoryEntry labelEntry = FatDirectoryEntry.createVolumeLabel(type, volumeLabel);
    ...
    protected final void read() throws IOException {
        ...
        for(int i = 0; i < getCapacity(); i++) {
            final FatDirectoryEntry e = FatDirectoryEntry.read(type, data, isReadOnly());
    ...
    public FatDirectoryEntry createSub(Fat fat) throws IOException {
        ...
        final ClusterChainDirectory dir = new ClusterChainDirectory(chain, false);
