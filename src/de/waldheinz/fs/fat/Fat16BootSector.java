...
final class Fat16BootSector extends BootSector {
     ...
     * partition having not enough sectors, so it would be misinterpreted as
     * FAT12 without special handling.
     *
     ...
     *
     * @param device
     *            the {@code BlockDevice} holding the boot sector
     */
     ...
     *             if the specified label is longer than
     *             {@link #MAX_VOLUME_LABEL_LENGTH}
     */
    public void setVolumeLabel(String label) throws IllegalArgumentException {
        ...
        for(int i = 0; i < MAX_VOLUME_LABEL_LENGTH; i++) {
            set8(VOLUME_LABEL_OFFSET + i, i < label.length() ? label.charAt(i) : 0);
        }
    }
    ...
    public FatType getFatType() {
        ...
        final long dataSectors = getSectorCount()
                - (getNrReservedSectors() + (getNrFats() * getSectorsPerFat()) + rootDirSectors);
     ...
     *
     * @param v
     *            the new number of entries in the root directory
     * @throws IllegalArgumentException
     *             for negative values
     */
