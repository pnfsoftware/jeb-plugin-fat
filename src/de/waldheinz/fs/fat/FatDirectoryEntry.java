...
final class FatDirectoryEntry extends AbstractFsObject {
    ...
    /**
     * The magic byte denoting that this entry was deleted and is free for
     * reuse.
     *
     * @see #isDeleted()
     */
     ...
     * The buffer must have at least {@link #SIZE} bytes remaining. The entry is
     * read from the buffer's current position, and if this method returns
     * non-null the position will have advanced by {@link #SIZE} bytes,
     * otherwise the position will remain unchanged.
     *
     * @param buff
     *            the buffer to read the entry from
     * @param readOnly
     *            if the resulting {@code FatDirecoryEntry} should be read-only
    ...
    public static FatDirectoryEntry createVolumeLabel(FatType type, String volumeLabel) {
        ...
        System.arraycopy(volumeLabel.getBytes(ShortName.ASCII), 0, data, 0, volumeLabel.length());

        final FatDirectoryEntry result = new FatDirectoryEntry(type, data, false);
    ...
    public void setCreated(long created) {
        LittleEndian.setInt16(data, 0x0e, DosUtils.encodeTime(created));
        LittleEndian.setInt16(data, 0x10, DosUtils.encodeDate(created));

        this.dirty = true;
    }

    public long getLastModified() {
        return DosUtils.decodeDateTime(LittleEndian.getUInt16(data, 0x18), LittleEndian.getUInt16(data, 0x16));
    }

    public void setLastModified(long lastModified) {
        LittleEndian.setInt16(data, 0x16, DosUtils.encodeTime(lastModified));
        LittleEndian.setInt16(data, 0x18, DosUtils.encodeDate(lastModified));

        this.dirty = true;
    }

    public long getLastAccessed() {
        return DosUtils.decodeDateTime(LittleEndian.getUInt16(data, 0x12), 0); /*
                                                                                * time
                                                                                * is
                                                                                * not
                                                                                * recorded
    ...
    public void setLastAccessed(long lastAccessed) {
        LittleEndian.setInt16(data, 0x12, DosUtils.encodeDate(lastAccessed));

        this.dirty = true;
    }

    /**
     * Returns if this entry has been marked as deleted. A deleted entry has its
     * first byte set to the magic {@link #ENTRY_DELETED_MAGIC} value.
     * 
     ...
     * 
     * @param length
     *            the new size of the file represented by this entry
     * @throws IllegalArgumentException
     *             if {@code length} is out of range
     */
    ...
    public long getStartCluster() {
        if(type == FatType.FAT32) {
            return (LittleEndian.getUInt16(data, 0x14) << 16) | LittleEndian.getUInt16(data, 0x1a);
        }
        else {
     ...
     *
     * @param buff
     *            the buffer to write this entry to
     */
    ...
    /**
     * Returns if the read-only flag is set for this entry. Do not confuse this
     * with {@link #isReadOnly()}.
     *
